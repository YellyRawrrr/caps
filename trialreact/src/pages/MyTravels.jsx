...
export default function MyTravels() {
  ...
  <nav className="flex space-x-6 text-sm font-medium text-gray-500 relative">
    <button
    ...
    <button
      ...
      className="relative pb-2 border-b-2 border-transparent hover:text-blue-600 hover:border-blue-500"
    >
      Rejected Orders
      {orders.some(order => order.status?.toLowerCase().includes('rejected')) && (
        <span className="absolute top-0 right-0 -mt-1 -mr-2 inline-flex h-2 w-2 rounded-full bg-red-500"></span>
      )}
    </button>
  </nav>
</div>
