export default function IdentificationStep({ formData, handleChange, setFormData }) {
  const options = [
  ...
  return (
    <>
      ...
      {/* Evidence Upload - Only show when IMMEDIATE is selected */}
      {formData.mode_of_filing === 'IMMEDIATE' && (
        <div className="mb-4">
          <label className="block mb-1 text-sm font-medium text-gray-700">
            Upload Evidence (Image/PDF)<span className="text-red-500 ml-1">*</span>
          </label>
          <input
            type="file"
            accept="image/*,.pdf"
            onChange={(e) => {
              if (e.target.files.length > 0) {
                setFormData((prev) => ({
                  ...prev,
                  evidence: e.target.files[0],  // store File object
                }));
              }
            }}
          ...
          <p className="text-xs text-gray-500 mt-1">
            Required for immediate filing. Supported formats: JPG, PNG, PDF
          </p>
        </div>
      )}
    </>
